...
static esp_err_t auth_request(const char *path, const char *form, http_response_t *response)
{
    ...
    if (n < 0 || (size_t)n >= sizeof(url)) { memset(response, 0, sizeof(*response)); return ESP_ERR_INVALID_SIZE; }
...
static esp_err_t refresh_access(auth_client_t *auth, unsigned *retry_after)
{
    ...
    *retry_after = response.retry_after;
    ...
    if (err == ESP_OK) {
        ...
        if (response.status == 200) err = accept_tokens(auth, root, true);
...
{
    if (!auth || !retry_after) return ESP_ERR_INVALID_ARG;
    *retry_after = 0;
    if (auth_client_ready(auth)) return ESP_OK;
    esp_err_t err = refresh_access(auth, retry_after);
    if (err == ESP_ERR_NOT_FOUND) err = device_login(auth, retry_after);
    return err;
}
