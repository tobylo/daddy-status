                       ...
                       http_response_t *response)
{
    ...
    memset(response, 0, sizeof(*response));
    if (!url || !*url || (token && !bearer_token_valid(token)))
        return ESP_ERR_INVALID_ARG;
    ...
    if (err == ESP_OK && token) {
        if (asprintf(&authorization, "Bearer %s", token) < 0)
            err = ESP_ERR_NO_MEM;
        else
    ...
    if (err == ESP_OK && form) {
        ...
        if (err == ESP_OK)
            err = esp_http_client_set_post_field(client, form, strlen(form));
    }
    if (err == ESP_OK)
        err = esp_http_client_perform(client);
    response->status = esp_http_client_get_status_code(client);
    if (err == ESP_OK &&
        (response->body.overflow || !esp_http_client_is_complete_data_received(client))) {
