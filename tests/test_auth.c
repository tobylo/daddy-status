...
#define DEVICE "{\"device_code\":\"a+b&c\",\"message\":\"Log in\",\"interval\":5,\"expires_in\":900}"
#define TOKENS "{\"access_token\":\"access\",\"refresh_token\":\"rotated\",\"token_type\":\"Bearer\",\"expires_in\":3600}"
#define ACCESS_ONLY "{\"access_token\":\"new-access\",\"token_type\":\"Bearer\",\"expires_in\":3600}"
...
void nvs_close(nvs_handle_t handle) { assert(handle && handles); --handles; }
esp_err_t nvs_get_str(nvs_handle_t handle, const char *key, char *value, size_t *size)
{
    ...
    if (wrong_type) return ESP_ERR_NVS_TYPE_MISMATCH;
    if (forced_size) { assert(!value); *size = forced_size; return ESP_OK; }
    if (!stored) return ESP_ERR_NVS_NOT_FOUND;
    size_t required = strlen(stored) + 1;
    if (value) { assert(*size >= required); memcpy(value, stored, required); }
    *size = required;
    return ESP_OK;
}
...
{
    assert(handle && !strcmp(key, "refresh_token"));
    secret_free(stored); stored = strdup(value);
    return ESP_OK;
}
esp_err_t nvs_erase_key(nvs_handle_t handle, const char *key)
{
    assert(handle);
    if (!strcmp(key, "access_token")) { old_access = false; return ESP_OK; }
    assert(!strcmp(key, "refresh_token"));
    secret_free(stored); stored = NULL; forced_size = 0; wrong_type = false;
    return ESP_OK;
}
esp_err_t nvs_commit(nvs_handle_t handle) { assert(handle); return commit_error; }
...
esp_err_t http_request(const char *url, const char *form, const char *token, http_response_t *response)
{
...
{
    assert(!handles);
    secret_free(stored); stored = NULL; old_access = true;
    forced_size = 0; storage_error = 0; query_error = 0; commit_error = 0; wrong_type = false;
    now = 0; sleep_count = 0; request_count = 0; script = steps; script_count = count;
}
...
static void device_and_refresh(void)
{
    ...
    reset(steps, 5);
    auth_client_t auth = {0}; unsigned retry;
...
static void revocation_and_transient(void)
{
    ...
    reset(throttled, 1); stored = strdup("keep-me");
...
static void invalid_and_missing_storage(void)
{
    ...
    query_error = 0; forced_size = TOKEN_LIMIT + 2; commit_error = ESP_FAIL;
    assert(token_storage_read(&token) == ESP_FAIL && !token && !handles);
    commit_error = 0; storage_error = ESP_FAIL;
    assert(token_storage_read(&token) == ESP_FAIL && !token && !handles);
}
...
static void terminal_auth_errors(void)
{
    ...
    for (unsigned i = 0; i < 3; ++i) {
        char body[96]; snprintf(body, sizeof(body), "{\"error\":\"%s\"}", errors[i]);
        const exchange_t steps[] = {{"/devicecode", 200, DEVICE, ESP_OK, 0}, {"/token", 400, body, ESP_OK, 0}};
        reset(steps, 2); auth_client_t auth = {0}; unsigned retry;
...
{
    const struct { const char *activity; presence_t expected; } cases[] = {
    ...
    for (unsigned i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        cJSON *root = cJSON_CreateObject(); cJSON_AddStringToObject(root, "activity", cases[i].activity);
        assert(presence_from_json(root) == cases[i].expected); cJSON_Delete(root);
    }
    assert(presence_from_json(NULL) == PRESENCE_UNKNOWN);
    cJSON *root = cJSON_Parse("{\"activity\":3}");
    assert(presence_from_json(root) == PRESENCE_UNKNOWN); cJSON_Delete(root);
}

int main(void)
{
    device_and_refresh(); revocation_and_transient(); invalid_and_missing_storage();
    terminal_auth_errors(); presence_mapping(); reset(NULL, 0);
